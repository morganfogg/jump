...
# NOTE: This particular version is intended for a regular Fish environment on Linux or Mac. If you are running a Bash
# environment on Windows, see either sh/wsl.sh or sh/cygwin.sh
...
function jump
    ...
    switch "$argv[1]"
        case "-c"
            if [ -z "$argv[2]" ]
                echo "Specify a name for the bookmark"
                return 1
            end
        ...
        case "*"
            ...
            if [ -z "$MATCH" ]
                ...
                cd (printf "%s" "$MATCH" | tr -d '\r'); or return 1;
            end
    end
end
